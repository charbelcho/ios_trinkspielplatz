                                            ...
                                            ForEach($spieler.items){ $item in
                                                HStack{
                                                    if spielerBearbeiten == false {
                                                        Text("\(item.name) setzt \(item.schlucke) Schluck/e auf \(item.zeichen)")
                                                    }
                                                    else {
                                                        VStack {
                                                            HStack {
                                                                ...
                                                                TextField("Schlucke" ,value: $item.schlucke, formatter: NumberFormatter())
                                                                    .textFieldStyle(RoundedBorderTextFieldStyle())
                                                            ...
                                                            Picker("Wähle ein Zeichen", selection: $item.zeichen) {
                                                                ForEach(zeichen, id: \.self) {
                                                                    Text($0)
                                                                }
                                                            }
                                    ...
                                    VStack(alignment: .leading) {
                                        ...
                                        HStack {
                                            Text("Schlucke")
                                                .padding(.leading)
                                                .frame(width: geo.size.width * 0.25)
                                            TextField("Schlucke", value: $schlucke, formatter: NumberFormatter()).textFieldStyle(RoundedBorderTextFieldStyle())
                                                .padding(.horizontal)
                                                .keyboardType(.numberPad)
                                        }
                                        
                                        Picker("Wähle ein Zeichen", selection: $selection) {
                                            ForEach(zeichen, id: \.self) {
                                                Text($0)
                                            }
                                        }
                                        ...
                                        HStack {
                                            Button(action: {
                                                if name.count < 1 {
                                                    ...
                                                    missing = true
                                                    alertTitle = "Keine Schlucke"
                                                    alertText = "Gib die Anzahl der Schlucke ein"
                                                }
                                                else if selection == "" {
                                                    missing = true
                                                    alertTitle = "Kein Zeichen ausgewählt"
                                                    alertText = "Wähle ein Zeichen aus"
                                                }
                                                else {
                                                    spieler.items.append(SpielerPferderennen(name: name, schlucke: schlucke, zeichen: selection))
                                                    name = ""
                                                    schlucke = 1
                                                    alertTitle = ""
                                                    alertText = ""
...
struct StartModalPferderennenView_Previews: PreviewProvider {
    static var previews: some View {
        StartModalPferderennenView(isStartModalOpen: .constant(false))
    }
}
